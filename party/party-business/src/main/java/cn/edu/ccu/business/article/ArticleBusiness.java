...
public class ArticleBusiness implements IArticle {
    ...
    public ArticleListResponse searchArticleList(String title, SplitPageRequest splitPageRequest) throws Exception {

        if (StringExtention.isTrimNullOrEmpty(title))
    ...
    //    List stringList = Arrays.asList(stringIntegerMap.keySet().toArray());
     ...
     //       List<Integer> articleIds = articleWordsIndexModelMapper.selectByKeyWords(stringList);
      ...
      //          if (articleModels == null || articleModels.size() == 0) {
             List<ArticleModel>    articleModels = articleModelMapper.selectArticleByLike(title, rowStart, pageSize);
          //      }
                ...
                if (splitPageRequest != null && splitPageRequest.isReturnCount()) {
                    ...
                    int rowCount;
                 //   if (articleModels == null || articleModels.size() == 0) {
                 //       rowCount = articleModelMapper.selectArticleByLikeCount(title);
                 //   } else {
                        rowCount = articleModelMapper.selectArticleByIdsCount1(title);
                  //  }
...
//            }
    ...
    public boolean delArticle(Integer articleId, Integer userId) throws Exception {
          String articleURLIndex = "http://192.168.248.246:8080/party-index/article/" + articleId;
          String articleURLSchool = "http://192.168.248.246:8080/party-school-index/article/" + articleId;
    //    ArticleModel articleModel = new ArticleModel();
   ...
   //     articleModel.setStatus(ArticleStatus.ADMIN_DELETE);
        ...
        int i = articleModelMapper.deleteByPrimaryKey(articleId);
         List list1 =  (List)bannerConfigModelMapper.selectBannerByURL(articleURLIndex);
        List list2 =  (List)bannerConfigModelMapper.selectBannerByURL(articleURLSchool);
        if(list1.size() > 0){
            bannerConfigModelMapper.deleteBannerByURL(articleURLIndex);
        }
        if(list2.size() > 0){
            bannerConfigModelMapper.deleteBannerByURL(articleURLSchool);
        }
        return i > 0;
    }
