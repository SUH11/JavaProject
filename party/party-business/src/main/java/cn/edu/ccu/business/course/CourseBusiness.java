...
public class CourseBusiness implements ICourse {
    ...
    public CourseListResponse listByPage(CourseListRequest courseListRequest, boolean withCourseWare) {
        ...
        if (courseModel != null) {
            ...
            if (IntegerExtention.hasValueAndMaxZero(courseModel.getCourseType()))
                map.put("courseType", courseModel.getCourseType());
        }
    ...
    @Override
    public Map selectCourse(int studentId) throws Exception{
        Map reMap = new HashMap();
       int sroleId = rStudentRoleModelMapper.selectSRoleId(studentId);
        List<Map> rcscourseIds = (List<Map>)rCourseStudentModelMapper.selectCourseId(studentId);
       List<Map> courseIds = (List<Map>)sroleCourseMapper.selectCoruseId(sroleId);
       List<CourseModel1> courseList = new ArrayList<CourseModel1>();
        for(Map courseId : courseIds){
            CourseModel1 courseModel1 =  courseModelMapper.selectCourse(courseId);
            CourseTypeModel courseTypeModel = courseTypeModelMapper.selectCourseType(courseModel1.getCourse_type());
            for(Map rcscouresId : rcscourseIds){
                if(rcscouresId.get("course_id") == courseId.get("course_id")){
                    courseModel1.setChoose(true);break;
                }else{
                    courseModel1.setChoose(false);
                }
            }
            courseModel1.setCourseTypeModel(courseTypeModel);
            courseList.add(courseModel1);
        }
        reMap.put("courseList", courseList);
