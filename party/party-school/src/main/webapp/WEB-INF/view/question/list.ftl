            ...
            <form id="searchForm">
                <div class="panel-body">
                    ...
                    <#if response.questionModelList??&&response.questionModelList?size gt 0>
                        ...
                        <tr>
                            <td>
                                <input type="button" class="btn btn-info" onclick="aaa('${question.id}');" id="question${question_index+1}" value="${question_index+1+(response.splitPageResponse.pageNo-1)*10}"></input>
                            </td>
                        </tr>
                        </#list>
                    <#else>
            ...
            <div id="content">
                    <#if response.questionModelList??&&response.questionModelList?size gt 0>
                        <#list response.questionModelList as question>
                            ...
                            <div class='col-xs-12'>
                            <span class='label label-info'>选项：</span><br>
                                <#assign text=question.answers />
                                <#assign json=text?eval />
                                <#assign i=0>
                                <#assign abcString="ABCDEFGHIJKLMNOPQRSTUVWXYZ">
                                <#if json??&&json?size gt 0>
                                    <#list json as item>
                                        <span class="label col-xs-1">
                                        ${abcString[item_index]}：
                                        </span>
                                        <div class="col-xs-11">
                                        ${item.name}
                                        </div>
                            ...
                            <#if json??&&json?size gt 0>
                                <#list json as item>
                                    <#if question.rightAnswer?index_of(item.value)!=-1>
                                        ${abcString[item.value?eval]}
                                    </#if>
                                </#list>
                            </#if>
                            </span>
