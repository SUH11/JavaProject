...
public class MainController extends BaseController {
    ...
    public ModelAndView index(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        Map map = iBannerConfig.selectBanner(1);
        List oneBanner = (List)map.get("oneBanner");
      if(oneBanner.size() > 0) {

           mav.addObject("oneBanner", oneBanner.get(0));
      }else{
          Map paraMap = new HashMap();
           paraMap.get(oneBanner);
           paraMap.put("name", null);
           paraMap.put("description", null);
           paraMap.put("url", null);
           mav.addObject("oneBanner",  paraMap);
       }
        return mav;
    }
...
//                && response.getArticleModelList().size() == 1
   //             && response.getSplitPage().getRecordCount()==1) {
    ...
    @RequestMapping(value = "/listDjBanner")
    public ModelAndView list_djbanner(
            HttpServletRequest httpRequest, SplitPageRequest pageRequest) throws Exception {


        ModelAndView mav = super.getModelAndView("BannerList", httpRequest);
       // pageRequest.setReturnCount(true);
