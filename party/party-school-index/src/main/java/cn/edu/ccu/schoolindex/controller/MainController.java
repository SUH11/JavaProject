...
//                && response.getArticleModelList().size() == 1
    ...
    public ModelAndView search(String title, SplitPageRequest splitPageRequest, HttpServletRequest request, HttpServletResponse response) throws Exception {

        ModelAndView mav = super.getModelAndView("search", request);

        splitPageRequest.setReturnCount(true);
        if(null != title && !title.equals("")) {
            ArticleListResponse articleListResponse = iArticle.searchArticleList(title, splitPageRequest);
